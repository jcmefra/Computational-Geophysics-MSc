...
@jit(nopython=True)
def propagator1(m,src,Ix0,Iz0,dx,dz,dt,max_offset,frec):
    ...
    for it in range(1,Nt-1):
        ...
        for iq in range(min(Ix0),max(Ix0)+1):
            P[iq,Iz0-1,it+1] = P[iq,Iz0-1,it+1] + src[it+1, iq-min(Ix0)]
    ...
    if Ix0.size > 1:
        Pt = np.reshape(P[Ix0-1,Iz0-1,:],(Ix0.size,Nt)).T
    else:
        if Ix0 > max_ix:
            Pt = np.reshape(P[np.arange(21,181,1)-1,Iz0-1,:],(165,Nt)).T
        else:
            Pt = np.reshape(P[np.arange(21,181,1),Iz0-1,:],(164+1,Nt)).T
